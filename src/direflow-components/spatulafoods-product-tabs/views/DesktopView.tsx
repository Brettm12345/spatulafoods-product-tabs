...
export const DesktopView: FC<ViewProps> = ({product}) => (
  <Tab.Group>
    <Tab.List className="flex mx-auto my-6 p-1 bg-brand-100 rounded-xl space-x-1">
      {Object.keys(sections).map(tab => (
        <Tab
          key={`${tab}-tab`}
          className={({selected}) =>
            classNames(
              'py-2.5 w-full text-brand-700 text-sm font-bold tracking-wider leading-5 rounded-xl select-none uppercase transition-colors ease-linear',
